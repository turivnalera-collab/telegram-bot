...
async def from_where(update: Update, ctx):
    ...
    phones=[['iOS 🍎'],['Android 🤖']]
    await update.message.reply_text("Какой у вас телефон?",
...
async def phone(update: Update, ctx):
    ...
    await update.message.reply_text("На какую игру нужен софт? 🎮",
...
async def game(update: Update, ctx):
    ...
    c=ctx.user_data.get("country","—"); p=ctx.user_data.get("phone","—")
...
async def panel(update: Update, ctx):
    ...
    data=q.data
    if data=="turn_on": set_active(True);  text="✅ Основной бот включён."
    elif data=="turn_off": set_active(False); text="🚫 Основной бот выключен."
    else: st="🟢 ВКЛЮЧЕН" if is_active() else "🔴 ВЫКЛЮЧЕН"; text=f"📊 Статус: {st}"
    kb=[[InlineKeyboardButton("🟢 Включить",callback_data="turn_on"),
         InlineKeyboardButton("🔴 Выключить",callback_data="turn_off")],
        [InlineKeyboardButton("📊 Статус",callback_data="status")]]
